    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            if(arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLoginLogger.info('ユーザによって、キャンセルされました。')
                return
            }
            ...
            setOverlayContent(
                'エラーが発生しました。',
                'Microsoft 認証に失敗しました。 もう一度やり直してください。',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>使用中のアカウント &#10004;' : '>Select Account'}</button>
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/irushiaTeam/Irushia_Launcher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'リリース ノートを読み込めませんでした。'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('GitHub からダウンロード<span style="font-size: 10px;color: gray;text-shadow: none !important;">Close the launcher and run the dmg to update.</span>', false, () => {
